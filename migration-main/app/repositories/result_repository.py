...
def _get_available_columns(table: str) -> set[str]:
    return set(_get_column_data_lengths(table).keys())
...
def get_pending_jobs() -> list[SqlInfoJob]:
    ...
    has_new_correct_columns = all(
        col in available_columns for col in ("TOBE_CORRECT_SQL", "BIND_CORRECT_SQL", "TEST_CORRECT_SQL")
    )

    select_correct_cols = (
        "TOBE_CORRECT_SQL, BIND_CORRECT_SQL, TEST_CORRECT_SQL"
        if has_new_correct_columns
        else "CAST(NULL AS VARCHAR2(4000)) AS TOBE_CORRECT_SQL, CAST(NULL AS VARCHAR2(4000)) AS BIND_CORRECT_SQL, CAST(NULL AS VARCHAR2(4000)) AS TEST_CORRECT_SQL"
    )
...
def get_feedback_corpus_rows(correct_kind: str, limit: int = 2000) -> list[dict[str, str]]:
    ...
    correct_column = _CORRECT_COLUMN_MAP.get(normalized_kind)
    if not correct_column:
        raise ValueError(f"Unsupported correct SQL kind: {correct_kind}")

    available_columns = _get_available_columns(table)
    if correct_column.upper() not in available_columns:
        return []
